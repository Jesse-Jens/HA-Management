...
def _tunnel_pid_file():
    return os.path.join(current_app.config['BASEDIR'], 'cloudflared.pid')
...
def cloudflare():
    ...
    if request.method == 'POST':
        if 'start' in request.form:
            _start_tunnel()
            flash('Tunnel gestart')
            return redirect(url_for('main.cloudflare'))
        if 'stop' in request.form:
            _stop_tunnel()
            flash('Tunnel gestopt')
            return redirect(url_for('main.cloudflare'))
        hostname = request.form.get('hostname') or 'homeassistant.jayjaysrv.com'
        port = request.form.get('port') or '5000'
        token = request.form.get('token', '')
        cfg['cloudflare'] = {'hostname': hostname, 'port': port, 'token': token}
        with open(cfg_path, 'w') as f:
            cfg.write(f)
        flash('Cloudflare configuratie opgeslagen')
        return redirect(url_for('main.cloudflare'))
    hostname = cfg.get('cloudflare', 'hostname', fallback='homeassistant.jayjaysrv.com')
    port = cfg.get('cloudflare', 'port', fallback='5000')
    token = cfg.get('cloudflare', 'token', fallback='')
    running = _tunnel_running()
    return render_template('cloudflare.html', hostname=hostname, port=port, token=token, running=running)
