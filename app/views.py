...
def _next_ip():
    used = {c.ip_address for c in Client.query.all() if c.ip_address}
...
def _generate_wg_keys(client):
    if not client.wg_private_key or not client.wg_public_key:
...
def _ha_request(client, method, path, **kwargs):
    """Send a request to the Home Assistant API for the given client."""
    if not client.ha_token:
        raise ValueError("Missing HA credentials")
    import requests
    ha_url = client.ha_url or 'http://localhost:8123'
...
def dashboard():
    ...
    counts = {'healthy': 0, 'attention': 0, 'critical': 0, 'offline': 0}
    for c in clients:
        try:
            resp = _ha_request(c, 'GET', '/api/')
...
def client_add():
    ...
    if request.method == 'POST':
        ...
        client = Client(
            ...
            ha_url=request.form.get('ha_url') or 'http://localhost:8123',
            ha_token=request.form.get('ha_token'),
        )
...
def client_edit(client_id):
    ...
    if request.method == 'POST':
        ...
        client.ha_url = request.form.get('ha_url') or 'http://localhost:8123'
...
def client_action(client_id, cmd):
    ...
    try:
        if cmd == 'restart':
            _ha_request(client, 'POST', '/api/services/homeassistant/restart')
        elif cmd == 'shutdown':
            _ha_request(client, 'POST', '/api/services/homeassistant/stop')
        flash('Command sent')
...
@main_bp.route('/client/<int:client_id>/wgconfig')
@login_required
def client_wgconfig(client_id):
    client = Client.query.get_or_404(client_id)
    _generate_wg_keys(client)
    if not client.ip_address:
        client.ip_address = _next_ip()
        db.session.commit()
    _append_peer(client.wg_public_key, f"{client.ip_address}/32")
    server_pub = ''
    try:
        with open('/etc/wireguard/server_publickey') as f:
            server_pub = f.read().strip()
    except Exception:
        pass
    endpoint = 'vpn-ha-management.jayjaysrv.com'
    port = '51820'
    cfg = f"""[Interface]
...
"""
    resp = current_app.response_class(cfg, mimetype='text/plain')
    fname = f"{client.name}_wg0.conf".replace(' ', '_')
