...
def cloudflare():
    ...
    if os.path.exists(cfg_path):
        cfg.read(cfg_path)
    labels = [('interface', 'ha-management.jayjaysrv.com'), ('vpn', 'vpn-ha-management.jayjaysrv.com')]
    if request.method == 'POST':
        label = request.form.get('label', 'interface')
        ...
        if 'stop' in request.form:
            ...
            return redirect(url_for('main.cloudflare'))
        default_host = dict(labels).get(label, 'ha-management.jayjaysrv.com')
        hostname = request.form.get('hostname') or default_host
        port = request.form.get('port') or '5000'
        token = request.form.get('token', '')
        if label not in cfg:
            cfg[label] = {}
        cfg[label]['hostname'] = hostname
        cfg[label]['port'] = port
        cfg[label]['token'] = token
        with open(cfg_path, 'w') as f:
            cfg.write(f)
        flash(f'{label.capitalize()} configuratie opgeslagen')
        return redirect(url_for('main.cloudflare'))
    data = {}
